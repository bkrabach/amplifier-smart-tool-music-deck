...
description: >
  ...
  is currently fresh; use deterministic commands for exact ID-targeted work.
use_cases:
...
- A verified read may succeed with exit 0 and no playlist. A write acknowledgement is reported as `acknowledged`, not verified until a relevant read confirms it. Incomplete or unknown work returns the existing `partial_result` envelope with its `completeness` record.
