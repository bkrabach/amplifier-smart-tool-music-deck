...
2. **A verb is model-backed or it needs no provider — and `--help` says
   which.** Every verb that is not model-backed runs with no provider
   configured and no provider SDK installed, including any added later. `check`
   additionally succeeds (exit 0) with no credentials, no network, in a fresh
   directory: the smoke test, where reporting problems IS success.
3. **A model-backed verb refuses before any prompt is built.** Without a usable
   substrate it exits 3, naming the missing precondition — SDK, provider, or
   credentials — and the fix. Never a silent deterministic fallback, and never
   a partial prompt sent to find out.
...
6. **The refusal vocabulary is frozen, in `contracts/refusals.v1.md`.** A
   caller branches on `error.code`; that set is a surface of its own and grew
   on its own schedule, so it is its own contract. Every code the tool emits
...
- **2026-09-06 — ratified ("do it all").** Cores 2 and 3 stop naming `plan` as
  the only model-backed verb: the boundary removal that day made a second one
  the point. Core 6's list moved out to `contracts/refusals.v1.md` — verbatim,
  no promise changed — because it had forced a trim on this contract at every
  amendment for three days, and the golf was starting to cost meaning.
- **2026-09-06 — ratified ("take it all the way live").** Core 4 turns on what
  the output is FOR; Core 1 handed it the stdout rule; Core 8 became
  proportional; Core 6 gained `port_unavailable`, then `cancelled` when a
